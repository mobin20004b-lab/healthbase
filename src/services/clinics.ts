...
export async function getClinicsByIds(ids: string[], locale: string = 'fa'): Promise<ClinicWithRelations[]> {
    ...
    try {
        const clinics = await prisma.clinic.findMany({
            ...
            include: {
                ...
                favoritedBy: false // No user context for now
            }
        });

        // Apply translations and calculate ratings
        return clinics.map((clinic) => {
            ...
            const averageRating = clinic.reviews.length > 0 ? totalRating / clinic.reviews.length : 0;
            const reviewCount = clinic.reviews.length;
            const isFavorited = false; // No user context
        ...
        console.warn("Database operation failed, returning mock data.", error);
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return MOCK_CLINICS.filter(c => ids.includes(c.id)) as any as ClinicWithRelations[];
    }
}
