...
import type { Clinic, Service, ClinicTranslation, ServiceTranslation, Review, Category, Specialty, Insurance, SpecialtyTranslation, InsuranceTranslation, CategoryTranslation } from '@prisma/client';
...
const MOCK_CLINICS = [
  {
    ...
    reviewCount: 120,
    services: [
        { id: 's1', name: 'Cardiology Consultation', priceMin: 5000000, priceMax: 10000000, currency: 'IRR', category: 'Consultation' }
    ],
    translations: [],
    reviews: [
        { id: 'r1', rating: 5, comment: 'Excellent service', user: { name: 'Ali' }, createdAt: new Date() }
    ],
    favoritedBy: [],
    insurances: [],
    specialties: []
...
export async function getClinicsByIds(
    ...
    try {
        ...
        return MOCK_CLINICS.filter(c => ids.includes(c.id)) as unknown as ClinicWithRelations[];
    }
}
...
export async function getClinicById(
    ...
    try {
        ...
        const services = clinic.services.map((service) => {
            ...
            const categoryName = cTranslation?.name || service.category?.name || undefined;
        ...
        const insurances = clinic.insurances.map(i => ({
            ...i,
            name: i.translations[0]?.name || i.name,
            translations: undefined
        }));

        const specialties = clinic.specialties.map(s => ({
            ...s,
