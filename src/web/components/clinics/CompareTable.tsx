    ...
    return (
        <div className="w-full overflow-x-auto pb-4">
            ...
            >
                ...
                <div className="font-bold text-on-surface p-4 flex items-center bg-surface/50 sticky left-0 backdrop-blur-sm z-20 border-r border-outline-variant/10">
                    Clinic
                </div>
                {clinics.map(clinic => (
                ...
                {/* Rating Row */}
                <div className="font-semibold text-on-surface-variant p-4 bg-surface-container-low/50 flex items-center sticky left-0 backdrop-blur-sm z-20 border-r border-outline-variant/10">
                    Rating & Wait
                </div>
                {clinics.map(clinic => {
                    const details = getClinicComparisonDetails(clinic.id);
                    return (
                ...
                {/* Availability Row */}
                <div className="font-semibold text-on-surface-variant p-4 flex items-center sticky left-0 bg-surface/50 backdrop-blur-sm z-20 border-r border-outline-variant/10">
                    Next Available
                </div>
                {clinics.map(clinic => {
                    const details = getClinicComparisonDetails(clinic.id);
                ...
                {clinics.map(clinic => {
                 ...
                 {/* Services Row */}
                 <div className="font-semibold text-on-surface-variant p-4 flex items-start pt-4 sticky left-0 bg-surface/50 backdrop-blur-sm z-20 border-r border-outline-variant/10">
                    Services
                </div>
                {clinics.map(clinic => (
